<#
Version 2.2
	added checks for correct AZ modules 
	auto-select if onyl one subscription 

Requires Powershell 7.1 or later. 

Copy this next to the JSON file.
...
.PARAMETER TemplateFile
  Path to the ARM template (default: .\NET-CSP-Lighthouse-ps.json)
...
.EXAMPLE
  .\Deploy-Lighthouse-Bulk.ps1 `
     -CustomerTenantId "aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee" `
     -SubscriptionListPath ".\subs.csv" 
...
param(
  [Parameter()][string]$TemplateFile = ".\NET-CSP-Lighthouse-ps.json",
...
# Check for the JSON template file
...
# Try to preview managedByTenantId & authorizations for operator awareness
try {
  $templateJson = Get-Content -LiteralPath $TemplateFile -Raw | ConvertFrom-Json
  $managedByTenantId = $templateJson.variables.managedByTenantId
  $auths = $templateJson.variables.authorizations
  if ($managedByTenantId) {
    Write-Host "==> Template managedByTenantId: $managedByTenantId" -ForegroundColor Yellow
  }
  if ($auths) {
...
# Connect to tenant with owner rights 
...
# only 1 subscription, so select it and continue
$SelectedSubs = $AllSubs
} 
elseif ($AllSubs.count -gt 1){
# popup for user to select which subscriptions to include
$SelectedSubs = $AllSubs | Out-GridView -Title "Select Subscription(s) to process" -PassThru	
} else {
# no subscriptions 
...
# Validate at least one subscription selected to process
